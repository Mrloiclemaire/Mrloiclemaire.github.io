...
const Map = ({
  ...
  mapboxgl.accessToken =
    "pk.eyJ1IjoiYWxleGtlcmJsYSIsImEiOiJja3kzYWlxbmYwMDg1MnhvbjV6MHBiOHplIn0.k3LrOobdBO8e_hnBQq6Z7Q";
  ...
  useEffect(() => {
    ...
    const flightCoordinate = flights.map((flight) => ({
      longitude: flight[5],
      latitude: flight[6],
      flightNumber: flight[1].replace(/ /g, ""),
      departure: flight[2],
      rotation: flight[10],
      altitude: flight[13],
      speed: flight[9],
    ...
    for (let i = 0; i < flightCoordinate.length; i++) {
      if (!!countryFilter && flightCoordinate[i].departure === countryFilter) {
        ...
        const marker = new mapboxgl.Marker(el)
          .setLngLat([
            flightCoordinate[i].longitude,
            flightCoordinate[i].latitude,
          ])
          ...
          .setPopup(
            new mapboxgl.Popup().setHTML(
              `<h4>Flight number : ${
                flightCoordinate[i].flightNumber
                  ? flightCoordinate[i].flightNumber
                  : "Unknown"
              }</h4><h4>Current altitude : ${
                flightCoordinate[i].altitude
                  ? flightCoordinate[i].altitude + " m"
                  : "Unknown"
              } </h4><h4>Speed : ${
                Math.floor(flightCoordinate[i].speed * 3.6)
                  ? Math.floor(flightCoordinate[i].speed * 3.6) + " Km/h"
                  : "Unknown"
              }</h4>`
            )
          )
        ...
        !!flightNumberFilter &&
        flightCoordinate[i].flightNumber === flightNumberFilter
      ) {
        const el = document.createElement("div");
        el.className = "marker";
        el.style.backgroundImage = `url(https://d29fhpw069ctt2.cloudfront.net/icon/image/49621/preview.svg)`;
  ...
  const toggleArrow = () => {
    arrowStatus === "arrow down"
      ? setArrowStatus("arrow up")
      : setArrowStatus("arrow down");
    formRadioStatus === "formRadio close"
      ? setFormRadioStatus("formRadio open")
      : setFormRadioStatus("formRadio close");
  };
  ...
  return (
    <div>
      <div className="map">
        ...
        <div className="button" onClick={toggleArrow}>
          Map settings<i className={arrowStatus}></i>
        </div>
